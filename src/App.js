...
function corsRewrite(url) {
    ...
    if (url.match(githubRegex)) {
        ...
        return url.replace(
            "https://community.octoprint.org/uploads/short-url",
            "https://bundleviewer.octoprint.org/bundles/forum"
        );
    }
    return url;
}
            ...
            <StyledEngineProvider injectFirst>
                <ThemeProvider theme={darkModeTheme}>
                    <SnackbarProvider
                        ref={notistackRef}
                        action={(key) => (
                            <Button onClick={dismissSnack(key)}>Close</Button>
                        )}
                        maxSnack={10}
                        anchorOrigin={{
                            vertical: "bottom",
        ...
        if (error) {
            ...
            return (
                <Alert severity="error">
                    ...
                    Could not load file. Make sure you are trying to load either an
                    OctoPrint systeminfo bundle, a zip containing logs, or a plain text
                    log file. Other files are not supported.
                </Alert>
            );
        } else if (loading) {
            ...
            return <NothingLoaded onUpload={handleUpload} />;
        }
    };
