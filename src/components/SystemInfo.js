...
export default function SystemInfo(props) {
    ...
    const checkLastSafeMode = () => {
        ...
        if (last_safe_mode_date.trim() === "unknown") {
            enqueueSnackbar("Last start in safe mode is unknown.", {
                key: "last_safe_mode",
                variant: "warning",
                persist: true
            });
        ...
        const formattedDiff = timeutils.formatTimeDiff(
            timeutils.diffInSeconds(last_safe_mode_date_obj, generation_date_obj)
        );

        enqueueSnackbar(
            `Last start in safe mode was ${formattedDiff} before bundle creation.`,
            {key: "last_safe_mode", variant: "info", persist: true}
        );
    };
    ...
    const throttled = checkField(
        ...
        (value) => value.trim() !== "0x0"
    );
    const throttle_check_disabled = checkField(
        "env.plugins.pi_support.throttle_check_enabled",
        (value) => !checkBoolean(value)
    );
    const marlin_bugfix = checkField("printer.firmware", (value) =>
        value.includes("Marlin bugfix-")
    );
    const unrecommended_model = checkField("env.plugins.pi_support.model", (value) =>
        checkModel(value)
    );
    const still_python_2 = checkField(
        "env.python.version",
        (value) => !semver.satisfies(value, ">=3.7")
    );
    ...
    if (throttle_check_disabled) {
        enqueueSnackbar(
            "Throttle check disabled by user. Potential undervoltage or overheating issues will not be detected.",
            {variant: "warning", persist: true, key: "throttle_check_disabled"}
        );
    }
    if (marlin_bugfix) {
        enqueueSnackbar(
            "Marlin-Bugfix firmware development build detected, firmware may be unstable.",
            {key: "marlin_bugfix"}
        );
    }
    if (unrecommended_model) {
        enqueueSnackbar("Unrecommended Raspberry Pi model detected.", {
            key: "unrecommended_model",
            variant: "error",
            persist: true
        });
    }
    if (still_python_2) {
        enqueueSnackbar("OctoPrint is still running under Python 2", {
            key: "still_python_2",
            variant: "warning",
            persist: true
        });
    ...
    if (python_too_old) {
        enqueueSnackbar("Python version is too old for automatic updating.", {
            key: "python_too_old",
            variant: "error",
            persist: true
        });
    ...
    if (pip_too_old) {
        enqueueSnackbar("Pip version is too old for automatic updating.", {
            key: "pip_too_old",
            variant: "error",
            persist: true
        });
    ...
    if (setuptools_too_old) {
        enqueueSnackbar("Setuptools version is too old for automatic updating.", {
            key: "setuptools_too_old",
            variant: "error",
            persist: true
        });
    }
    if (issue_4392) {
        enqueueSnackbar("Susceptible for update/plugin install issue #4392", {
            key: "issue_4392"
        });
    }
    checkLastSafeMode();
    ...
    return (
        <Accordion defaultExpanded>
            <AccordionSummary
                ...
                aria-controls="panel-sysinfo-content"
                id="panel-sysinfo-header"
            >
                <div className={classes.accordionbar}>
                    <div className={classes.title}>
                        ...
                        >
                            System Information
                            {safemode ? (
                                <SafeModeIcon className={classes.icon} size="1.5em" />
                            ) : null}
                            {throttled ? (
